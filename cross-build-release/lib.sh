...
umountImageFile() {
  ...
  rm -rf $rootfs/install-scripts-omv/stageCache/*
  rm -rf $rootfs/install-scripts-omv/logs/*
  ...
  umount $rootfs/proc
  umount $rootfs/tmp
  umount $rootfs/install-scripts-omv/stageCache
...
function addLysmarineScripts() {
  ...
  chmod 0775 ${rootfs}/install-scripts-omv/install.sh
}
